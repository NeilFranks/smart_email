...
def add_account(creds, address, app_token):
    ...
    response = requests.post(
        ...
        json={"creds": stringCreds, "address": address},
    )
...
def modify_account(idx, creds, app_token):
    ...
    response = requests.put(
        "%s/api/et/%s/" % (baseURL(), idx),
        headers={"Authorization": app_token},
        json={"creds": stringCreds},
    )
    return response
...
def retrieve_accounts(app_token):
    ...
    response = requests.get(
        "%s/api/et/" % baseURL(), headers={"Authorization": app_token}
    )
    ...
    if response.status_code == 200:
        ...
        for entry in content:
            ...
            try:
                creds = pickle.loads(codecs.decode(credsString.encode(), "base64"))
                accountList.append({"id": idx, "address": address, "creds": creds})
            except ValueError:
